    ...
    <div className="flex-1 overflow-hidden flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-auto scrollable">
        ...
        <table id="vehicles-table" className="w-full border-collapse" style={{ minWidth: 3650 }}><thead className="sticky top-0 z-10"><tr>
          ...
          <td className={`${td} !p-0 sticky left-0 ${expanded === key ? "bg-[#F0F7FF]" : "bg-white"}`}><div className="flex min-h-[58px] items-stretch"><button type="button" aria-label={expanded === key ? "Скрыть поездки дня" : "Показать поездки дня"} aria-expanded={expanded === key} onClick={e => { e.stopPropagation(); toggle(row); }} className="flex w-12 shrink-0 cursor-pointer items-center justify-center border-r border-[#E5E7EB] text-2xl font-bold text-[#2563A6]">{expanded === key ? "▾" : "▸"}</button><span className="flex items-center px-3">{dateRu(row.work_date)}</span></div></td>
