...
export default function VehiclesScreen({ refreshToken, onViewAnalytics }: Props) {
  ...
  useEffect(() => { setLoading(true); setError(""); api<any>(`reports/vehicles?${query({ date_from: dateFrom, date_to: dateTo, vehicle_id: vehicleId, active_only: onlyActive })}`).then(setData).catch(e => setError(e.message)).finally(() => setLoading(false)); }, [requestKey, refreshToken]);

  const rows = useMemo(() => data.rows.filter((row: any) => `${row.vehicle_name} ${row.vehicle_plate} ${row.driver} ${row.objects} ${row.crew}`.toLowerCase().includes(search.toLowerCase())), [data.rows, search]);
  ...
  return <div className="flex flex-col h-full">
    <div className="bg-white border-b border-[#E5E7EB] px-6 py-4"><div className="flex flex-wrap items-end gap-3">
      ...
      <Button variant="secondary" onClick={() => { setDateFrom(monthAgo()); setDateTo(today()); setVehicleId(""); setOnlyActive(true); setSearch(""); setRequestKey(v => v + 1); }}>Сбросить</Button>
    </div></div>
    <div className="bg-[#F3F4F6] px-6 py-4 border-b border-[#E5E7EB]"><div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
      ...
      <KpiCard label="Строк в отчёте" value={rows.length} />
    </div></div>
    <div className="flex-1 overflow-hidden flex flex-col bg-white">
      <div className="flex items-center justify-between px-6 py-3 border-b border-[#E5E7EB]"><span className="text-sm text-[#6B7280]">Показано: {rows.length} строк</span><div className="flex items-center gap-3"><SearchInput placeholder="Найти машину, водителя или объект" value={search} onChange={e => setSearch(e.target.value)} className="w-72" /><Button variant="secondary" onClick={() => { exportTable("vehicles-table", "автомобили", "Отчёт по автомобилям"); setToast("Таблица XLS скачана"); }} icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" stroke="currentColor" strokeWidth="1.5"/><path d="M12 10v7m0 0-3-3m3 3 3-3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}>Выгрузить XLS</Button></div></div>
      <div className="flex-1 overflow-auto scrollable">
        ...
        <table id="vehicles-table" className="w-full border-collapse" style={{ minWidth: 3650 }}><thead className="sticky top-0 z-10"><tr>
          ...
          <td className={td} data-no-export><div className="flex flex-col items-start gap-1"><button onClick={() => toggle(row)} className="text-[#2563A6] hover:underline font-medium">{expanded === key ? "Скрыть поездки" : "Поездки дня"}</button><button onClick={() => onViewAnalytics(String(row.vehicle_id))} className="text-[#2563A6] hover:underline">Аналитика</button><span className="text-[10px] text-[#9CA3AF]">Эпизодов: {row.segment_count || row.movement_segments || 0}</span></div></td>
